...
import positionGeneration as pg
...
parser.add_argument('--num_groups',default=1,type=int,
    help="The number of groups which should be formed.")
parser.add_argument('--margin_groups',default=6.0,type=float,
    help="The minimum margin between the center of groups")
parser.add_argument('--within_group_distance_min',default=0.4,type=float,
    help="The minimum distance of object whithin a group")
parser.add_argument('--within_group_distance_max',default=3.0,type=float,
    help="The maximum distance of object whithin a group")

#---------------group by similarity
parser.add_argument('--group_by_similarity',default=0,type=int,
    help="If its 1, we generate images where we need to distinguise bwtween groups by similarity.")
parser.add_argument('--group_by_postition',default=0,type=int,
    help="Turns on group by position.")

#---------------group by closure
parser.add_argument('--group_by_closure', default=0, type=int,
      help="Turns on group by closure.")
#---------------group by continuity
parser.add_argument('--group_by_continuity', default=0, type=int,
      help="Turns on the group by continuity.")

#----------------group by good figure
parser.add_argument('--group_by_good_figure', default=0, type=int,
      help="Turns on group by good figure.")
...
def check_center_is_valid(centers, new_center):
  print("Check if position is valid")
  minmargin = args.margin_groups
  for pos in centers:
    distance = math.sqrt((pos[0] - new_center[0]) ** 2 + (pos[1] - new_center[1]) ** 2)
    if distance < minmargin:
      return False
  return True


def get_center_positions_spatial_grouping(num_groups):
  print("Calculate centers for groups")
  centers = []
  tries=0
  minx=-7
  maxx=7
  ...
  for group in range(num_groups):
    ...
    while not check_center_is_valid(centers,(x,y)):
      x = random.uniform(minx, maxx)
      y = random.uniform(miny, maxy)
      tries=tries+1
      if tries>1000:
        print("Number of groups and margin does not match. Please lower the number of groups or the margin of the groups")
        return get_center_positions_spatial_grouping(num_groups)
    centers.append((x,y))
  print(centers)
  return centers
...
def add_random_objects_continuity(scene_struct, args, camera):
  ...
  generated_pos = pg.pos_by_continiuty(num_groups)
  ...
  if not all_visible:
    ...
    for obj in blender_objects:
      utils.delete_object(obj)
    return add_random_objects_continuity(scene_struct, args, camera)

  return objects, blender_objects

def add_random_objects_good_figure(scene_struct, args, camera):
  # Load the property file
  with open(args.properties_json, 'r') as f:
    properties = json.load(f)
    color_name_to_rgba = {}
  ...
  generated_pos = pg.pos_by_good_fig(num_groups)
  # pg.position_visualizer(generated_pos)
  for group_index in range(num_groups):
    group = generated_pos[group_index]
    for position in group:
  ...
  if not all_visible:
    ...
    for obj in blender_objects:
      utils.delete_object(obj)
    return add_random_objects_good_figure(scene_struct, args, camera)
...
if __name__ == '__main__':
  if INSIDE_BLENDER:
    ...
    args = parser.parse_args(argv) #<class 'argparse.Namespace'>
    main(args)
